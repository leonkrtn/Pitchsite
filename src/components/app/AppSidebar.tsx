...
function UpgradeModal({ locale, onClose }: { locale: string; onClose: () => void }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.55)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
      <div style={{ background: '#fff', borderRadius: '20px', width: '100%', maxWidth: '820px', boxShadow: '0 32px 100px rgba(0,0,0,.25)', animation: 'dropIn 200ms ease-out', overflow: 'hidden' }}>
        {/* Header */}
        ...
        {/* Plans */}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px', padding: '24px 32px 32px' }}>
          {plans.map(plan => (
